...
bool sendTcpMessage(const sockaddr_in& targetAddr, const string& msg) {
    ...
    int sent;
    if (msg == "mypeer") { // 사용자의 정보를 전송하기 위해 mypeer라고 입력하면 송신자를 제외한 모든 클라이언트에게 전송
        //여기에 초기에 송신자 피어에 설정한 이름을 넣도록 하면 될 듯, 그러기 위해선 아래 반복문 for문에 수정해야할 듯
    }
    // 데이터 전송
...
int main(int argc, char* argv[]) {
    ...
    // ===============================
    // 1. 초기 설정 입력 + mypeer 설정
    // ===============================
    cout << "[설정] 내(로컬) 포트 번호를 입력하세요: ";
    cin >> local_port;
    mypeer.port = port = local_port;

    cout << "[설정] 통신할 사용자(피어)의 수를 입력하세요: ";
    cin >> peerCount;

    // mypeer 초기 설정
    cout << "[설정] 내(로컬) 이름을 입력하세요: ";
    cin >> mypeer.name;

    cout << "[설정] 내(로컬) IP 주소를 입력하세요: ";
    cin >> mypeer.ipStr;

    memset(&mypeer.addr, 0, sizeof(mypeer.addr));
    mypeer.addr.sin_family = AF_INET;
    mypeer.addr.sin_port = htons(mypeer.port);
    if (inet_pton(AF_INET, mypeer.ipStr.c_str(), &mypeer.addr.sin_addr) <= 0) {
        cerr << "잘못된 IP 주소: " << mypeer.ipStr << "\n";
        return 1;
    }
